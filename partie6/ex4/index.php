...
// Instructions
// Test de la présence des paramètres 'lastname' et 'firstname'
    // if (!empty($_GET['language']) && !empty($_GET['server'])) {
    //     $message = 'langage : ' . $_GET['language'] . ' serveur : ' . $_GET['server'];
    // }
    // // Sinon signalement par une erreur
    // else {
    //     $message = 'Il manque au moins une information';
    // }

// Autre méthode
if (empty($_GET['language'])) {
    $errorLanguage = 'Il manque la langue !';
} else {
    $returnMessageLanguage = 'C\'est bien mon gars t\'as bien renseigné la langue : ' .  $_GET['language'];
}

if (empty($_GET['server'])) {
    $errorServer = 'Il manque le nom du serveur !';
} else {
    $returnMessageServer = 'C\'est bien mon gars t\'as bien renseigné le nom du serveur : ' .  $_GET['server'];
}
    ...
    <div class="mt-2">
        <form action="#" method="get" class="d-flex flex-column align-items-center" id="loginForm">
            ...
            <input type="text" name="age" id="age" class="inputForm" placeholder="age" 
                value="<?=$errorLanguage??''?> ; <?=$returnMessageLanguage??''?> ; <?=$errorServer??''?> ; <?=$returnMessageServer??''?>" 
                readonly>
        </form>
    </div>
    <!-- Form end -->
